...
```{r wd}
## setwd("C:/Users/usuario/Desktop/ikiam_2022/bioinformatica/")

```

### [0.5 PUNTOS] 1.2. Cargue la data "mRNA_expr.csv"
```{r data}
setwd("C:/Users/usuario/Desktop/proyectos_2022/bioinformatica/examen_final/Evelyn_Vasquez")
df <- "cargue la data"
library(readr)
...
## Función long_data, input una variable (data frame) que presenta ordenamiento wider y out una variable (data frame) que presenta ordenamiento longer 
library(tidyr)
library(dplyr)
long_df <- function(df) {
  #df$dataset <- as.factor(df$dataset)
  long <- gather(df, gen, expresion_level, GATA3, PTEN, XBP1, ESR1, MUC1, FN1, GAPDH, factor_key=TRUE)
  long_1 <- select(long, -bcr_patient_barcode)
  return (long_1)
}
```
...
## Seleccion de datos, seleccionamos el cancer 
## Primero se realiza una seleccion de datos, en este caso elegimos Uterine Corpus Endometrial Carcinoma UCEC. Leuego creamos el df desc_table que transforma los datos de long a wider mediante el comando pivot_wider 

library(sjPlot)
library(sjmisc)

UCEC_df <- df_long %>% filter(dataset == "UCEC") %>% select(2:3) 
## Uterine Corpus Endometrial Carcinoma
desc_table <- UCEC_df %>% group_by(gen) %>% mutate(row = row_number()) %>%
  tidyr::pivot_wider(names_from = gen, values_from = expresion_level) %>%
  select(-row)
descr(desc_table) %>% tab_df(title = "Estadistica descriptiva del cancer de utero UCEC y el nivel de expresion de siete genes", file = "results/mRNA_expr_summary.doc") 
```
...
```{tex}
# la tabla presenta un resumen de calculos de estadisticas descriptivas de los niveles de expresion de los sietes genes en el data set UCEC (cancer de utero). En la columna n se puede observar que existen 54 individuos, el rango de media es de {-4.12:2.17} indicando que los conjuntos de valores de cada gen difieren. Finalemente en la columna de error estandar presentar valores menores 0.26 indicando una estimacion precisa de la media de las muestras. 
...
library(ggplot2) 
## creamos la función tcga_boxplots. El parametro de entrada es un data frame y la salida es un grafico del nivel de expresion de cada tipo de cancer 
...
## creamos una lista de data frame con cada tipo de cancer 
lista <- list(
...
cancertype_boxplots = lapply(lista, function(x) {ggplot(x, aes(dataset, expresion_level, col = dataset)) +
    geom_boxplot() + labs( y = "Nivel de expresión")})
...
## Creamos un data frame para cada tipo de cancer 
BRCA <- filter(df_long, dataset == "BRCA")
OV <- filter(df_long, dataset == "OV")
LUSC <- filter(df_long, dataset == "LUSC")
KIPAN <- filter(df_long, dataset == "KIPAN")
...
## creamos la lista plots_cancer. La lista contiene data frames en formato wider 
plots_cancer <- list (  
...
## La función reg_gen_expression(). El parametro de entrada es un gen de estudio que posteriormete la función se encargara de crear las 6 combinaciones posibles. Las 6 combinaciones son por cada tipo de cancer, obteniendo de esta manera 36 graficos finales 
reg_gen_expression <- function (gen) {
## Escribimos un enunciado condicional para graficar unicamente los graficos que involucren al parametro gen de entrada 
  if (gen == "GATA3"){
    total <- list (
...
```{r list_regplots}
# Escriba el código para generar la lista. 
## La producción de graficas seran del gen FN1

list_regplots <- reg_gen_expression("FN1")
...
list_regplots[[5]][[2]]
## El 11vo grafico es "[[5]][[2]]". El primer valor es la combinacion de genes y el primer indica el tipo de cancer
ggsave("results/regresion11.pdf", width = 5, height = 8, dpi = 400)
```
...
## boxplot3.png es un grafico de caja y bigotes. El grafico del cancer de pulmon indica que hubieron valores de niveles de expresión positivos y negativos la media oscila entre 0.7 y 0.9. Los valores 3 y -5 fueron el mas alto y bajo respectivamente. 

## regresion11 Indica una grafico de dispersion de los niveles de expresion del gen FN1 en el eje x y MUC1 en el eje y. Los valores de dispersion no indican una marcada tendencia y la linea de regresion lineal confirma debido a que limitado numero de datos estan cerca de la linea de regrersión.
```
